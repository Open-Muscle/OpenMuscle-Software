# Hardware Open Muscle Labler - (LASK5) V2
...
defaults = {
    'device_name': 'OpenMuscle Labeler V2',
...
batt_level.atten(machine.ADC.ATTN_11DB)
...
def calculate_battery_percentage(batt_level):
    # Convert ADC reading to voltage
    adc_voltage_uv = batt_level.read_uv()
    
    # Adjust for voltage divider & corrected for actual measured values
    correction_factor = 4.127 / 3.83  # ≈ 1.077
    actual_voltage = (adc_voltage_uv * 2 * correction_factor) / 1_000_000  # Adjust for divider & µV → V
    ...
    min_battery_voltage = 2.7  # Voltage when battery is nearly empty

    # Clamp and scale to 0–100%
    if actual_voltage <= min_battery_voltage:
        percentage = 0
    else:
        percentage = ((actual_voltage - min_battery_voltage) /
...
def fastRead():
    ...
    TARGET_RATE = 100  # Target sample rate in Hz
    ...
    while True:
        ...
        if time.ticks_diff(now, last_send_time) >= SEND_INTERVAL * 1000:
            ...
            for i, val in enumerate(averaged_data):
                span = maxes[i] - mins[i] or 1          # avoid ÷0
                norm = (val - mins[i]) / span          # 0.0 … 1.0
                norm = max(0.0, min(norm, 1.0))        # clamp
                calibrated.append(norm)
            # ------------------------------------------------------------------
            # Prepare the packet
            packet = {
                "id": "OM-LASK5",
            ...
            try:
                nm.udp_fast_send('192.168.1.49', 3141, raw_data)
                print('Sent:', raw_data)
...
def ESPNowSend():
    ...
    time.sleep(0.3)  # debounce after selecting this mode
    
    while True:
        data_f = []
        
        # Scale the 4 hall sensor readings to 0–800
        for i in range(len(cells)):
            raw = cells[i].read()
            span = maxes[i] - mins[i]
            if span <= 0:
                span = 1  # avoid division by zero
            # Normalize to 0.0–1.0, then scale to 0–800
            norm = (raw - mins[i]) / span
            norm = max(0.0, min(norm, 1.0))  # clamp
            scaled = int(norm * 800)
            data_f.append(scaled)
        
        # Joystick (thumb) — optional: scale to 0–800 too
        joy_raw = joystick_x.read()  # 0–4095
        joy_scaled = int((joy_raw / 4095.0) * 800)
        data_f.append(joy_scaled)
...
def mainloup(pi=pi,plen=plen,led=led,cells=cells,start=start,select=select,up=up,down=down,count=count):
    ...
    while not exit_bool:
        ...
        if select.value() == 0:
            ...
            button_thresh += -1
        if button_thresh > 200:
            exit_bool = True
            print('exit_bool:',exit_bool)
